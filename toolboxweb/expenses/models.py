...
class PushSubscription(models.Model):
    ...
    Created when the user grants notification permission and the frontend
    subscribes with the VAPID public key. Multiple devices per user are
    supported (each endpoint is unique to the browser + device combo).
    """
...
def notify(user, title, body='', kind='split', link=''):
    ...
    try:
        ...
        return None
    # Fire Web Push to every subscribed device — best-effort, non-blocking.
    try:
